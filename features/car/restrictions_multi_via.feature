...
Feature: Car - Multiple Via Turn restrictions
    ...
    @no_turning
    Scenario: Car - Multiple via restriction with non-compressable via nodes
        Given the node map
            """
            a b c d e f g h
                i   j   k
    ...
    @no_turning
    Scenario: Car - Expensive unrestricted route vs multiple via restriction route
        Given the node map
            """
            a                            b
    ...
    Scenario: Car - Junction with multiple via restriction to side road, via traffic lights
    ...
    Scenario: Car - Many overlapping restrictions, traffic signals
    # Example: https://www.openstreetmap.org/#map=19/48.76987/11.43410
        Given the node map
            """
            p8_____a_______n________o_5x
            1      |                |
            |       \              /
            r___q____b____        m
            |         \   \___  _/
       ...
       And the nodes
        ...
        When I route I should get
            | from | to | route                |
            | 1    | 6  | pr,rst,tv,tv         |
            | 1    | 3  | pr,rqb,bcd,de,eh,eh  |
            | 1    | 4  | pr,rqb,bcd,de,ei,ei  |
            | 1    | 7  | pr,rqb,bl,lk,kw,kw   |
            | 1    | 8  |                      |
            | 2    | 6  |                      |
            | 2    | 3  | fe,eh,eh             |
            | 2    | 4  | fe,ei,ei             |
            | 2    | 7  | fe,ejk,kw,kw         |
            | 2    | 8  | fe,de,bcd,ba,ap,ap   |
            | 3    | 6  | eh,de,dgt,tv,tv      |
            | 3    | 4  | eh,ei,ei             |
            | 3    | 7  | eh,ejk,kw,kw         |
            | 3    | 8  | eh,de,bcd,ba,ap,ap   |
            | 4    | 6  | ei,de,dgt,tv,tv      |
            | 4    | 3  | ei,eh,eh             |
            | 4    | 7  | ei,ejk,kw,kw         |
            | 4    | 8  | ei,de,bcd,ba,ap,ap   |
            | 5    | 6  | xo,oml,ld,dgt,tv,tv  |
            | 5    | 3  | xo,oml,ld,de,eh,eh   |
            | 5    | 4  | xo,oml,ld,de,ei,ei   |
            | 5    | 7  |                      |
            | 5    | 8  | xo,on,na,ap,ap       |


    Scenario: Car - Multiple via restriction start and end on same node
    # Example: https://www.openstreetmap.org/#map=19/52.41988/16.96088
       ...
       And the nodes
        ...
        When I route I should get
            | from | to | route           |
            | a    | g  | enter,right,up,left,down,right,up,left,exit,exit |
            | b    | a  | enter,right,up,left,down,right,up,left,exit,exit |
        # This is a correct but not within the spirit of the restriction.
        # Does this indicate the restriction is not strong enough?


    Scenario: Car - Multi via restriction to prevent bypassing slow main road ("rat runs")
    ...
    Scenario: Car - Multi via restriction with to,via,from sharing same node
    ...
    Scenario: Car - Multiple via restriction exception applied
    ...
    Scenario: Car - Multiple via restriction respects exception
    # Example: https://www.openstreetmap.org/#map=19/50.04920/19.93251
        Given the node map
            """
            a  b   c   d  e
