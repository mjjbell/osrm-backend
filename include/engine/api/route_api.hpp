...
class RouteAPI : public BaseAPI
{
  public:
                 ...
                 util::json::Object &response) const
    {
        ...
        for (const auto &route : raw_routes.routes)
        {
            ...
            jsRoutes.values.push_back(MakeRoute(route.route_endpoints,
  ...
  protected:
    ...
    MakeRoute(flatbuffers::FlatBufferBuilder &fb_result,
              const std::vector<PhantomEndpoints> &route_endpoints,
              const std::vector<std::vector<PathData>> &unpacked_path_segments,
              const std::vector<bool> &source_traversed_in_reverse,
              const std::vector<bool> &target_traversed_in_reverse) const
    {
        auto legs_info = MakeLegs(route_endpoints,
    ...
    util::json::Object MakeRoute(const std::vector<PhantomEndpoints> &route_endpoints,
                                 const std::vector<std::vector<PathData>> &unpacked_path_segments,
                                 const std::vector<bool> &source_traversed_in_reverse,
                                 const std::vector<bool> &target_traversed_in_reverse) const
    {
    ...
    MakeLegs(const std::vector<PhantomEndpoints> &route_endpoints,
             const std::vector<std::vector<PathData>> &unpacked_path_segments,
             const std::vector<bool> &source_traversed_in_reverse,
             const std::vector<bool> &target_traversed_in_reverse) const
    {
        ...
        auto number_of_legs = route_endpoints.size();
        legs.reserve(number_of_legs);
        leg_geometries.reserve(number_of_legs);

        for (auto idx : util::irange<std::size_t>(0UL, number_of_legs))
        {
            const auto &phantoms = route_endpoints[idx];
