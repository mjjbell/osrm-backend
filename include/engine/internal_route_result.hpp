...
struct InternalRouteResult
{
    ...
    std::vector<PhantomEndpoints> route_endpoints;
                                                       ...
                                                       const std::vector<bool> &is_waypoint)
{
    ...
    BOOST_ASSERT(leggy_result.route_endpoints.size() > 1);
    ...
    for (auto i : util::irange<std::size_t>(0, leggy_result.unpacked_path_segments.size()))
    {
        if (is_waypoint[i])
        {
            ...
            collapsed.route_endpoints.push_back(leggy_result.route_endpoints[i]);
        ...
        // no new leg, collapse the next segment into the last leg
        {
            ...
            BOOST_ASSERT(!collapsed.route_endpoints.empty());
            collapsed.route_endpoints.back().target_phantom =
                leggy_result.route_endpoints[i].target_phantom;
            ...
            if (!leggy_result.unpacked_path_segments[i].empty())
            {
                ...
                last_segment[old_size].weight_until_turn +=
                    ...
                    leggy_result.source_traversed_in_reverse[i]
                        ? leggy_result.route_endpoints[i].source_phantom.reverse_weight
                        : leggy_result.route_endpoints[i].source_phantom.forward_weight;

                last_segment[old_size].duration_until_turn +=
                    leggy_result.source_traversed_in_reverse[i]
                        ? leggy_result.route_endpoints[i].source_phantom.reverse_duration
                        : leggy_result.route_endpoints[i].source_phantom.forward_duration;
            }
        }
    }

    BOOST_ASSERT(collapsed.route_endpoints.size() == collapsed.unpacked_path_segments.size());
    return collapsed;
}
} // namespace engine
} // namespace osrm
