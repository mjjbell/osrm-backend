...
class BasePlugin
{
  protected:
    ...
    {
        std::cout << " We start with " << phantom_node_pair_list.size() << " snapped phantom nodes " << std::endl;
        const auto check_component_id_is_tiny =
            [](const std::pair<PhantomNodeCandidates, PhantomNodeCandidates> &phantom_pair) {
                return std::all_of(phantom_pair.first.begin(), phantom_pair.first.end(), [](const PhantomNode& phantom) {
                    return phantom.component.is_tiny;
                });
            };

        // are all phantoms from a tiny cc?
        const auto check_all_in_same_component =
            [](const std::vector<std::pair<PhantomNodeCandidates, PhantomNodeCandidates>> &nodes) {
                return std::any_of(nodes.front().first.begin(), nodes.front().first.end(), [&](const PhantomNode& phantom) {
                  ...
                  return std::all_of(std::begin(nodes),
                                     ...
                                     [component_id](const PhantomNodePair &phantom_pair) {
                                         return std::any_of(phantom_pair.first.begin(), phantom_pair.first.end(), [&] (const PhantomNode &other_phantom) {
                                             return component_id == other_phantom.component.id;
                                         });
                                     });
                });
        ...
        const auto fallback_to_big_component =
            [](const std::pair<std::vector<PhantomNode>, std::vector<PhantomNode>> &phantom_pair) {
                if (std::all_of(phantom_pair.first.begin(), phantom_pair.first.end(), [&](const auto &phantom) {
                        return phantom.component.is_tiny;
                }))
                {
                    if (std::any_of(phantom_pair.second.begin(),
                                    phantom_pair.second.end(),
                                    [&](const auto &phantom) {
                                        return phantom.IsValid() && !phantom.component.is_tiny;
                                    }))
                    {
        ...
        std::vector<std::vector<PhantomNode>> snapped_phantoms;
        ...
        {
            std::for_each(phantom_node_pair_list.begin(),
                           phantom_node_pair_list.end(),
                           [&] (const PhantomNodePair &pair) {
                              snapped_phantoms.push_back(pair.first);
                });
        }
        else
        {
            std::for_each(phantom_node_pair_list.begin(),
                          ...
                          [&] (const PhantomNodePair &phantom_pair) {
                            if (std::all_of(phantom_pair.first.begin(), phantom_pair.first.end(), [&](const PhantomNode &phantom_node) {
                              return phantom_node.component.is_tiny;
                            }))
                            {
                                if (std::any_of(phantom_pair.second.begin(),
                                                phantom_pair.second.end(),
                                                ...
                                                }))
                                {
                                    ...
                                    snapped_phantoms.push_back(phantom_pair.second);
                                    return;
                                }
                            }

                            snapped_phantoms.push_back(phantom_pair.first);
                          });
        }

        std::cout << " We have " << snapped_phantoms.size() << " snapped phantom nodes " << std::endl;
                           ...
                           bool use_all_edges = false) const
    {
        ...
        for (const auto i : util::irange<std::size_t>(0UL, parameters.coordinates.size()))
        {
            ...
            if (use_hints && parameters.hints[i] && AreValidHints(*parameters.hints[i], parameters.coordinates[i], facade))
            {
                for (const auto& hint : *parameters.hints[i]) {
                    phantom_nodes[i].push_back(PhantomNodeWithDistance{
                        hint.phantom,
                        util::coordinate_calculation::haversineDistance(parameters.coordinates[i],
                                                                        hint.phantom.location)});
                };
                                                 ...
                                                 const api::BaseParameters &parameters) const
    {
        ...
        for (const auto i : util::irange<std::size_t>(0UL, parameters.coordinates.size()))
        {
            ...
            {
                for (const auto& hint : *parameters.hints[i]) {
                    phantom_node_pairs[i].first.push_back(hint.phantom);
                };
                // we don't set the second one - it will be marked as invalid
                continue;
            }
            ...
            std::cout << "For phantom pair " << i << " , we got " << phantom_node_pairs[i].first.size() << " first, and " << phantom_node_pairs[i].second.size() << " second" << std::endl;

            // we didn't find a fitting node, return error
            if (phantom_node_pairs[i].first.empty() || std::any_of(phantom_node_pairs[i].first.begin(), phantom_node_pairs[i].first.end(), [](const auto &phantom_node) {
                       return !phantom_node.IsValid();
            }))
            {
            ...
            std::cout << "After validity check, For phantom pair " << i << " , we got " << phantom_node_pairs[i].first.size() << " first, and " << phantom_node_pairs[i].second.size() << " second" << std::endl;
            BOOST_ASSERT(std::all_of(phantom_node_pairs[i].first.begin(), phantom_node_pairs[i].first.end(), [](const PhantomNode &phantom_node) {
                  return phantom_node.IsValid();
            }));
            BOOST_ASSERT(std::all_of(phantom_node_pairs[i].second.begin(), phantom_node_pairs[i].second.end(), [](const PhantomNode &phantom_node) {
                  return phantom_node.IsValid();
            }));
        }
                                           ...
                                           const std::vector<util::Coordinate> &coordinates) const
    {
        ...
        auto mismatch = std::mismatch(phantom_nodes.begin(),
                                      ...
                                      [](const auto &phantom_node, const auto &coordinate) {
                                          return std::any_of(phantom_node.first.begin(), phantom_node.first.end(), [&](const auto &phant) {return phant.input_location == coordinate;});
                                      });
