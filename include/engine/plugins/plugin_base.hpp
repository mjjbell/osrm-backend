...
class BasePlugin
{
  protected:
    ...
    SnapPhantomNodes(std::vector<PhantomCandidateAlternatives> alternatives_list) const
    {
                    ...
                    [&](const PhantomNode &phantom) {
                        ...
                        return std::all_of(
                            ...
                            [component_id](const PhantomCandidateAlternatives &alternatives) {
                                return HasComponent(alternatives.first, component_id);
                            });
                    });
            };
        ...
        std::vector<PhantomNodeCandidates> snapped_phantoms(alternatives_list.size());
                           ...
                           bool use_all_edges = false) const
    {
        ...
        {
            if (use_hints && !parameters.hints[i].empty() &&
                AreValidHints(parameters.hints[i], parameters.coordinates[i], facade))
            {
                std::transform(parameters.hints[i].begin(),
                               parameters.hints[i].end(),
                               std::back_inserter(phantom_nodes[i]),
                               [&](const auto &hint) {
                                   return PhantomNodeWithDistance{
                                       hint.phantom,
                                       util::coordinate_calculation::haversineDistance(
                                           parameters.coordinates[i], hint.phantom.location)};
                               });
                continue;
            }
            ...
            phantom_nodes[i] = facade.NearestPhantomNodesInRange(
                ...
                use_approaches ? parameters.approaches[i].get() : engine::Approach::UNRESTRICTED,
                use_all_edges);
        }
        ...
        for (const auto i : util::irange<std::size_t>(0UL, parameters.coordinates.size()))
        {
                ...
                AreValidHints(parameters.hints[i], parameters.coordinates[i], facade))
            {
                std::transform(parameters.hints[i].begin(),
                               ...
                               [](const auto &hint) { return hint.phantom; });
