        ...
        : BaseParametersGrammar::base_type(root_rule)
    {
        const auto add_hint = [](engine::api::BaseParameters &base_parameters,
            ...
            if (!hint_strings.empty())
            {
                ...
                for (const auto& hint_string : hint_strings) {
                    location_hints.template emplace_back(engine::Hint::FromBase64(hint_string));
                }
                base_parameters.hints.emplace_back(std::move(location_hints));
            }
            else
            {
