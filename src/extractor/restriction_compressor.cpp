    ...
    std::vector<UnresolvedManeuverOverride> &maneuver_overrides)
{
    ...
    auto index = [&](auto &element) {
        starts.insert(std::make_pair(element.via_restriction.from, &element));
        ends.insert(std::make_pair(element.via_restriction.to, &element));
        for (const auto& via: element.via_restriction.via) {
            vias.insert(std::make_pair(via, &element));
        }
    };
    // !needs to be reference, so we can get the correct address
    const auto index_starts_ends_vias = [&](auto &restriction) {
        index(restriction);
    };
...
void RestrictionCompressor::Compress(const NodeID from, const NodeID via, const NodeID to)
{
    ...
    const auto update_start = [&](auto ptr) {
        ...
        BOOST_ASSERT(ptr->via_restriction.from == via);
        if (ptr->via_restriction.via.front() == to)
        {
            ptr->via_restriction.from = from;
        }
        // ____ | to - p.from | via - p.via | from - p.to | ____
        else
        {
            BOOST_ASSERT(ptr->via_restriction.via.front() == from);
            ptr->via_restriction.from = to;
        }
    };
    ...
    const auto reinsert_start = [&](auto ptr) { starts.insert(std::make_pair(ptr->via_restriction.from, ptr)); };
    ...
    const auto update_end = [&](auto ptr) {
        BOOST_ASSERT(ptr->via_restriction.to == via);
        // p.from | ____ - p.via | from - p.to | via - ____ | to
        if (ptr->via_restriction.via.back() == from)
        {
            ptr->via_restriction.to = to;
        }
        // p.from | ____ - p.via | to - p.to | via - ____ | from
        else
        {
            BOOST_ASSERT(ptr->via_restriction.via.back() == to);
            ptr->via_restriction.to = from;
        }
    };
    ...
    const auto reinsert_end = [&](auto ptr) { ends.insert(std::make_pair(ptr->via_restriction.to, ptr)); };
    std::for_each(end_ptrs.begin(), end_ptrs.end(), reinsert_end);

    // extract all via ptrs and remove the compressed nodes
    // (We know this won't be the first or last via node)
    auto all_vias_range = vias.equal_range(via);
    std::vector<TurnRestriction *> via_ptrs;
    std::transform(all_vias_range.first,
                   all_vias_range.second,
                   std::back_inserter(via_ptrs),
                   [](const auto pair) { return pair.second; });

    const auto update_via = [&](auto ptr) {
      BOOST_ASSERT(std::find(ptr->via_restriction.via,via) != ptr->via_restriction.via.end());
      ptr->via_restriction.via.erase(std::remove(ptr->via_restriction.via.begin(), ptr->via_restriction.via.end(), via), ptr->via_restriction.via.end());
    };
    std::for_each(via_ptrs.begin(), via_ptrs.end(), update_via);
