...
RestrictionGraph::RestrictionRange RestrictionGraph::GetRestrictions(RestrictionID id) const
{
    ...
    return boost::make_iterator_range_n(restrictions.begin() + node.restrictions_begin_idx,
...
{
    const auto &node = nodes[id];
    return boost::make_iterator_range_n(edges.begin() + node.edges_begin_idx, node.num_edges);
}
